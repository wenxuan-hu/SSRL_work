...
class LiteDRAMCrossbar(Module):
    ...
    def do_finalize(self):
        ...
        """
        for nm,master_wdata_ready in enumerate(master_wdata_readys):
            master_wdata_ready_dly = TappedDelayLine(
                signal = master_wdata_ready,
                ntaps  = 32
            )
            self.submodules+=master_wdata_ready_dly
            master_wdata_readys[nm] = master_wdata_ready_dly.taps[self.write_latency]

        for nm,master_rdata_valid in enumerate(master_rdata_valids):
            master_rdata_valid_dly = TappedDelayLine(
                signal = master_rdata_valid,
                ntaps  = 32
            )
