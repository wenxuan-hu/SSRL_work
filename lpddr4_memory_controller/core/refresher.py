...
class RefreshSequencer(Module):
    ...
    """
    def __init__(self, cmd, trp, trfc, postponing):
        self.start = Signal()
        self.done  = Signal()
...
class RefreshPostponer(Module):
    ...
    def __init__(self, postponing=1):
        ...
        count = Signal(4,reset=postponing-1)
        self.sync += [
            self.req_o.eq(0),
            If(self.req_i,
                count.eq(count - 1),
...
class Refresher(Module):
    ...
    def __init__(self, settings, clk_freq, zqcs_freq=1e0, postponing=1):
        ...
        self.tRP=Signal(max=255,reset_less=True,name="ref_tRP_cfg")
        self.tRFC=Signal(max=255,reset_less=True,name="ref_tRFC_cfg")
        self.tREFI=Signal(max=4095,reset_less=True,name="ref_tREFI_cfg")
        self.postponing=Signal(4,reset_less=True,name="ref_POSTPONE_cfg")
        # # #
        ...
        timer = RefreshTimer(self.tREFI)
        self.submodules.timer = timer
        self.comb += timer.wait.eq(~timer.done)

        # Refresh Postponer ------------------------------------------------------------------------
        postponer = RefreshPostponer(self.postponing)
        ...
        # Refresh Sequencer ------------------------------------------------------------------------
        sequencer = RefreshSequencer(cmd, self.tRP, self.tRFC, self.postponing)
