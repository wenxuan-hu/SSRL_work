...
class Multiplexer(Module, AutoCSR):
    ...
    def __init__(self,
        ...
        self.tRRD=Signal(max=255,reset_less=True,name="mul_tRRD_cfg")
        ...
        if settings.phy.nphases == 1 or \
            (settings.phy.nphases == 4 and settings.phy.memtype=="LPDDR4"):
            print("1:4 LPDDR4 Memory or phase=1")
            self.rdphase=Signal(max=3,name="mul_rd_phase_cfg",reset_less=True)
            self.wrphase=Signal(max=3,name="mul_wr_phase_cfg")
            #rdphase = settings.phy.rdphase
            #wrphase = settings.phy.wrphase
            #print("rdphase=",rdphase)
            #print("wrpahse=",wrphase)
            self.rdcmdphase = Signal(max=3,name="mul_rdcmd_phase_cfg")
            self.wrcmdphase = Signal(max=3,name="mul_wrcmd_phase_cfg")
            #rdcmdphase=0
            #wrcmdphase=0
        else:
            print(settings.phy.nphases,settings.phy.memtype)
            self.rdphase = settings.phy.rdphase
            self.wrphase = settings.phy.wrphase
            if isinstance(self.rdphase, Signal):
                self.rdcmdphase = Signal.like(self.rdphase)
                self.comb += self.rdcmdphase.eq(self.rdphase - 1) # Implicit %nphases.
            else:
                self.rdcmdphase = (self.rdphase - 1)%nphases
            if isinstance(self.rdphase, Signal):
                self.wrcmdphase = Signal.like(self.wrphase)
                self.comb += self.wrcmdphase.eq(self.wrphase - 1) # Implicit %nphases.
            else:
                self.wrcmdphase = (self.wrphase - 1)%nphases
        ...
        if settings.phy.nphases == 1 or (settings.phy.nphases == 4 and settings.phy.memtype=="LPDDR4"):
        ...
        self.submodules.trrdcon = trrdcon = tXXDController(self.tRRD) #settings.timing.tRRD
        self.comb += trrdcon.valid.eq(choose_cmd.accept() & choose_cmd.activate())

        # tFAW timing (Four Activate Window) -------------------------------------------------------
        self.submodules.tfawcon = tfawcon = tFAWController(self.tFAW)
        ...
        self.submodules.tccdcon = tccdcon = tXXDController(self.tCCD)
        ...
        """
        self.submodules.twtrcon=twtrcon=tXXDController(self.WTR_LATENCY)
        self.comb += twtrcon.valid.eq(choose_req.accept() & choose_req.write())
        
        self.submodules.trtwcon=trtwcon=tXXDController(self.RTW_LATENCY)
        ...
        def steerer_sel(steerer, access):
            ...
            for i in range(nphases):
                ...
                if access == "read":
                    r.append(If(i == self.rdphase,If(choose_req.cmd.is_read,steerer.sel[i].eq(STEER_REQ))))
                    r.append(If(i == self.rdcmdphase,If(choose_req.cmd.is_cmd,steerer.sel[i].eq(STEER_REQ))))
                if access == "write":
                    r.append(If(i == self.wrphase,If(choose_req.cmd.is_write,steerer.sel[i].eq(STEER_REQ))))
                    r.append(If(i == self.wrcmdphase,If(choose_req.cmd.is_cmd,steerer.sel[i].eq(STEER_REQ))))
            return r

        # Control FSM ------------------------------------------------------------------------------
        # modify for lpddr4 1:4
